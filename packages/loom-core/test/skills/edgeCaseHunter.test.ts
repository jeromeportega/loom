...
const SKILL = 'edge-case-hunter';
const SOURCE_ID = SOURCE.EDGE_CASE;
...
describe('edge-case-hunter SKILL body', () => {
  ...
  it('replaces the BMAD bespoke output format with the shared findings schema', () => {
    const body = loadBody();
    // The vendored original emitted trigger_condition / guard_snippet /
    // potential_consequence — none of those bespoke keys survive the port.
    assert.ok(!/trigger_condition|guard_snippet|potential_consequence/.test(body));
  });
