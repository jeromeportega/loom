 ...
 *      architecture.md, epic.yaml) after promoteArtifacts runs. End-point is
 *      HEAD so any commit added by promoteArtifacts is caught.
...
function initialCommit(dir: string, message: string = 'initial'): string {
  ...
  return gitSafe(dir, ['rev-parse', 'HEAD']).output;
}
...
describe('target-repo code-only invariant — cases 1+2: code-only diff and ledger present', () => {
  ...
  let store: EpicStore;
  let baseSha: string;
  // artifactDir is resolved in before() once so Case 2 tests share a single source of truth.
  ...
  before(() => {
    ...
    // Resolve artifactDir once so all Case 2 tests share it.
    ...
    artifactDir = path.join(reposDir, slugs[0], epicId);
  });
  ...
  //
  // Diff end-point is HEAD (captured after promoteArtifacts) so any commit
  // inadvertently added by promoteArtifacts is included and triggers failure.

  it('case 1: git diff is code-only — exactly the expected code file and nothing else', () => {
    const diffRes = gitSafe(targetRepo, ['diff', '--name-only', baseSha, 'HEAD']);
    const files = diffRes.output.split('\n').filter(Boolean);
    assert.deepEqual(
      files,
      ['src/feature.ts'],
  ...
  it('case 1: target repo HEAD is unchanged after promoteArtifacts (no new commit on target)', () => {
    const epicBranchSha = gitSafe(targetRepo, ['rev-parse', 'epic/' + epicId]).output;
    const headNow = gitSafe(targetRepo, ['rev-parse', 'HEAD']).output;
    assert.equal(
      headNow,
      epicBranchSha,
  ...
  it('case 2: loom-home has at least two commits (initial + artifact commit)', () => {
    ...
    const count = parseInt(countRes.output, 10);
  ...
  it('case 2: loom-home commit includes artifacts under repos/<slug>/<epicId>/', () => {
    ...
    const committedFiles = lsRes.output.split('\n').filter(Boolean);
...
describe('target-repo code-only invariant — case 3: no history rewrite of pre-existing .loom_outputs commits', () => {
  ...
  before(() => {
    ...
    oldLoomOutputsSha = gitSafe(targetRepo, ['rev-parse', 'HEAD']).output;
  ...
  it('case 3: the pre-existing .loom_outputs commit SHA is unchanged in git history (no rebase)', () => {
    ...
    const allShas = logRes.output.split('\n').filter(Boolean);
    assert.ok(
  ...
  it('case 3: loom-home captures artifacts without touching target repo commits', () => {
    ...
    const caseArtifactDir = path.join(reposDir, slugs[0], epicId);
    assert.ok(
      fs.existsSync(path.join(caseArtifactDir, 'provenance.json')),
