...
import {
  resolveOpportunityEngineModels,
  DEFAULT_GATE_MODEL,
  DEFAULT_JUDGE_MODEL,
} from '../models.js';
...
describe('models — exported constants', () => {
  it('DEFAULT_GATE_MODEL is the haiku model (ADR-005)', () => {
...
describe('resolveOpportunityEngineModels — defaults when no opts and env unset', () => {
  ...
  it('judgeModel defaults to DEFAULT_JUDGE_MODEL', () => {
    const { judgeModel } = resolveOpportunityEngineModels();
...
describe('resolveOpportunityEngineModels — opts override env and defaults', () => {
  it('opts.gateModel wins over LOOM_EVAL_GATE_MODEL env', () => {
    process.env.LOOM_EVAL_GATE_MODEL = 'env-gate';
    const { gateModel } = resolveOpportunityEngineModels({ gateModel: 'opts-gate' });
    assert.equal(gateModel, 'opts-gate');
  });

  it('opts.gateModel wins over default when env is unset', () => {
    const { gateModel } = resolveOpportunityEngineModels({ gateModel: 'explicit-gate' });
  ...
  it('opts.judgeModel wins over LOOM_EVAL_JUDGE_MODEL env', () => {
    ...
    const { judgeModel } = resolveOpportunityEngineModels({ judgeModel: 'opts-judge' });
    assert.equal(judgeModel, 'opts-judge');
  });

  it('opts.judgeModel wins over default when env is unset', () => {
    const { judgeModel } = resolveOpportunityEngineModels({ judgeModel: 'explicit-judge' });
...
describe('resolveOpportunityEngineModels — env overrides default (second rung)', () => {
...
// ── opts undefined fields fall through correctly ──────────────────────────────

describe('resolveOpportunityEngineModels — undefined opts fields fall through correctly', () => {
  it('opts.gateModel undefined falls through to env', () => {
    process.env.LOOM_EVAL_GATE_MODEL = 'env-gate-fallthrough';
    const { gateModel } = resolveOpportunityEngineModels({ gateModel: undefined });
  ...
  it('opts.judgeModel undefined falls through to env', () => {
    ...
    const { judgeModel } = resolveOpportunityEngineModels({ judgeModel: undefined });
