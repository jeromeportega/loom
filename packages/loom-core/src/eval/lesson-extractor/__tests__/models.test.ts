...
describe('models — exported constants', () => {
  it('DEFAULT_GATE_MODEL is the production skill_gen model (ADR-005)', () => {
...
describe('resolveLessonExtractorModels — defaults when no opts and env unset', () => {
  it('gateModel defaults to DEFAULT_GATE_MODEL', () => {
    const { gateModel } = resolveLessonExtractorModels();
    assert.equal(gateModel, DEFAULT_GATE_MODEL);
  });

  it('judgeModel defaults to DEFAULT_JUDGE_MODEL', () => {
    const { judgeModel } = resolveLessonExtractorModels();
...
// ── opts take highest precedence (FR-7) ──────────────────────────────────────

describe('resolveLessonExtractorModels — opts override env and defaults (FR-7)', () => {
  it('opts.gateModel wins over LOOM_EVAL_GATE_MODEL env', () => {
    process.env.LOOM_EVAL_GATE_MODEL = 'env-gate';
    const { gateModel } = resolveLessonExtractorModels({ gateModel: 'opts-gate' });
    assert.equal(gateModel, 'opts-gate');
  });

  it('opts.gateModel wins over default when env is unset', () => {
    const { gateModel } = resolveLessonExtractorModels({ gateModel: 'explicit-gate' });
  ...
  it('opts.judgeModel wins over LOOM_EVAL_JUDGE_MODEL env', () => {
    ...
    const { judgeModel } = resolveLessonExtractorModels({ judgeModel: 'opts-judge' });
    assert.equal(judgeModel, 'opts-judge');
  });

  it('opts.judgeModel wins over default when env is unset', () => {
    const { judgeModel } = resolveLessonExtractorModels({ judgeModel: 'explicit-judge' });
...
describe('resolveLessonExtractorModels — env overrides default (second rung)', () => {
...
// ── opts.gateModel undefined falls through to env (edge case) ────────────────

describe('resolveLessonExtractorModels — undefined opts fields fall through correctly', () => {
  it('opts.gateModel undefined falls through to env', () => {
    process.env.LOOM_EVAL_GATE_MODEL = 'env-gate-fallthrough';
    const { gateModel } = resolveLessonExtractorModels({ gateModel: undefined });
  ...
  it('opts.judgeModel undefined falls through to env', () => {
    ...
    const { judgeModel } = resolveLessonExtractorModels({ judgeModel: undefined });
