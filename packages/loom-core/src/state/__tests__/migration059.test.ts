...
describe('v32 migration: SCHEMA_VERSION constant', () => {
  ...
  it('runMigrations() on a v31 DB advances schema_version to current (33)', () => {
    ...
    runMigrations(db);
    assert.equal(schemaVersion(db), 33, 'schema_version bumped to 33 after migration');
    db.close();
  });
});
...
describe('v32 migration: anchor seed is one-time (security — no self-heal)', () => {
  it('does NOT re-seed a deleted anchor on a steady-state v32 reopen', () => {
    ...
    runMigrations(db); // v31 -> current (v33) upgrade: crosses <32, so seeds the anchor once
