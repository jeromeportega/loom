...
async function captureText(options: Parameters<typeof runStatus>[0]): Promise<string> {
  const lines: string[] = [];
...
describe('loom status — revise_round = 0 (no tag)', () => {
  it('does not emit "(revise" tag when revise_round is 0', async () => {
...
describe('loom status — revise_round = 2 (tag present)', () => {
  it('emits "(revise 2)" tag when revise_round is 2', async () => {
    const db = createDatabase(path.join(repo, '.loom', 'loom.db'));
    new EpicStore(db).create('epic-076', 'Revise test epic');
    const agents = new AgentStore(db);
...
describe('loom status — revise tag position', () => {
  it('(revise 2) appears after (retry 1) and (recovered 1), before [model]', async () => {
    const db = createDatabase(path.join(repo, '.loom', 'loom.db'));
    ...
    const posModel     = storyLine.indexOf('[claude-');
...
describe('loom status — elapsed tag unaffected by revise tag', () => {
  it('elapsed-time token is still present alongside the revise tag', async () => {
    const db = createDatabase(path.join(repo, '.loom', 'loom.db'));
