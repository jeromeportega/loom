...
/** Minimal valid epic YAML with configurable stories. */
function makeEpicYaml(
  epicId: string,
  stories: Array<{
    id: string;
    ...
    .map((s) => {
      const depsList =
        (s.dependencies ?? []).length > 0
          ...
          : '\n    dependencies: []';
      const effortLine =
        s.estimatedEffort !== undefined ? `\n    estimated_effort: ${s.estimatedEffort}` : '';
      return `  - id: ${s.id}
    title: Story For Testing
    description: A story used in critical path tests.
    acceptance_criteria:
      - Criterion one${depsList}${effortLine}
    estimated_complexity: small`;
    })
    .join('\n');
  return `epic_id: ${epicId}
title: Critical Path Test Epic
priority: must-have
prd_ref: prd.md
requirements:
  - A requirement
...
describe('GET /api/fleet — criticalPath field', () => {
  let projectDir: string;
  let cpSrv: LaunchResult;

  beforeEach(async () => {
    projectDir = fs.mkdtempSync(path.join(os.tmpdir(), 'loom-fleet-cp-'));
    cpSrv = await launchFleet(50, projectDir);
  });
  ...
  it('returns criticalPath with chain and minutes when stories have estimated_effort', async () => {
    const { epicStore } = cpSrv;
    epicStore.create('epic-001', 'Epic with effort data');

    const yamlContent = makeEpicYaml('epic-001', [
      { id: 'story-001', estimatedEffort: 30, dependencies: [] },
      { id: 'story-002', estimatedEffort: 45, dependencies: ['story-001'] },
    ]);
    const yamlPath = 'epic-001.yaml';
    fs.writeFileSync(path.join(projectDir, yamlPath), yamlContent, 'utf8');
    epicStore.updatePaths('epic-001', { yaml_path: yamlPath });

    const res = await fetch(`${cpSrv.baseUrl}/api/fleet`, { headers: HEADERS });
    assert.equal(res.status, 200);
    const cards = (await res.json()) as FleetCard[];
    const card = cards.find((c) => c.epic_id === 'epic-001');
    assert.ok(card, 'card exists');
    assert.ok(card!.criticalPath !== null, 'criticalPath is not null');
    assert.ok(Array.isArray(card!.criticalPath!.chain), 'chain is an array');
    assert.ok(card!.criticalPath!.chain.length > 0, 'chain is non-empty');
    assert.ok(card!.criticalPath!.estimatedMinutes > 0, 'estimatedMinutes > 0');
    // The full chain story-001 → story-002 has weight 30 + 45 = 75
    assert.deepEqual(card!.criticalPath!.chain, ['story-001', 'story-002']);
    assert.equal(card!.criticalPath!.estimatedMinutes, 75);
  });

  it('returns criticalPath: null when no story has estimated_effort', async () => {
    const { epicStore } = cpSrv;
    epicStore.create('epic-002', 'Epic without effort data');

    const yamlContent = makeEpicYaml('epic-002', [
      { id: 'story-001', dependencies: [] },
      { id: 'story-002', dependencies: ['story-001'] },
    ]);
    const yamlPath = 'epic-002.yaml';
    fs.writeFileSync(path.join(projectDir, yamlPath), yamlContent, 'utf8');
    epicStore.updatePaths('epic-002', { yaml_path: yamlPath });

    const res = await fetch(`${cpSrv.baseUrl}/api/fleet`, { headers: HEADERS });
    const cards = (await res.json()) as FleetCard[];
    const card = cards.find((c) => c.epic_id === 'epic-002');
    assert.ok(card, 'card exists');
    assert.strictEqual(card!.criticalPath, null, 'criticalPath is null (no effort data)');
  });

  it('returns criticalPath when only some stories have estimated_effort (partial data)', async () => {
    const { epicStore } = cpSrv;
    epicStore.create('epic-003', 'Epic with partial effort data');

    const yamlContent = makeEpicYaml('epic-003', [
      { id: 'story-001', estimatedEffort: 60, dependencies: [] },
      { id: 'story-002', dependencies: ['story-001'] }, // no effort
      { id: 'story-003', estimatedEffort: 20, dependencies: ['story-001'] },
    ]);
    const yamlPath = 'epic-003.yaml';
    fs.writeFileSync(path.join(projectDir, yamlPath), yamlContent, 'utf8');
    epicStore.updatePaths('epic-003', { yaml_path: yamlPath });

    const res = await fetch(`${cpSrv.baseUrl}/api/fleet`, { headers: HEADERS });
    const cards = (await res.json()) as FleetCard[];
    const card = cards.find((c) => c.epic_id === 'epic-003');
    assert.ok(card, 'card exists');
    assert.ok(card!.criticalPath !== null, 'criticalPath is not null (some effort data exists)');
    assert.ok(Array.isArray(card!.criticalPath!.chain), 'chain is an array');
    assert.ok(card!.criticalPath!.chain.length > 0, 'chain is non-empty');
    // chain story-001 → story-003 has weight 60 + 20 = 80; story-001 → story-002 = 60 + 0 = 60
    assert.equal(card!.criticalPath!.estimatedMinutes, 80);
  });

  it('returns criticalPath: null for an epic with no yaml_path', async () => {
    const { epicStore } = cpSrv;
    epicStore.create('epic-004', 'Epic without yaml');
    // No yaml_path set — yaml_path is null

    const res = await fetch(`${cpSrv.baseUrl}/api/fleet`, { headers: HEADERS });
    const cards = (await res.json()) as FleetCard[];
    const card = cards.find((c) => c.epic_id === 'epic-004');
    assert.ok(card, 'card exists');
    assert.strictEqual(card!.criticalPath, null, 'criticalPath is null when yaml_path absent');
  });
});
