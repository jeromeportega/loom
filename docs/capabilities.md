...
| Capability | How to use | Notes |
|---|---|---|
| **Run signal scanners** | `loom scan` / `loom_scan_signals` | Runs three scanners concurrently — audit-log work-failure clusters, code-debt TODOs (capped at 200 deterministic matches), and GitHub issues (degrades to empty + audit note when `gh` is unavailable) — then makes a single batched LLM clustering call over the capped open-signal set to produce a ranked opportunity list. Writes one `signal_scan` audit row. |
