...
| **Plan via the weave intake path (Phase 0)** | `loom weave "<brief>"` | Same brief-quality gate, same Analyst → PM → Architect planner, and the same execution path as `loom epic`. In Phase 0 the two commands produce an identical epic; `loom epic` is unchanged. The P0 verdict is **observe-only**: it is recorded in the audit log but does not influence planning, gate decisions, or execution — planning and execution output is byte-identical whether a verdict is present or absent. The intake-classification layer (feature / bug / chore + size + confidence + rationale) will be wired in a future phase. |
| **Intake classification timeout** | `policy.agents.intake_classify_timeout_ms` (120_000–600_000 ms, default 180_000) | Wall-clock bound for the `loom weave` intake-classification LLM call. Default 180 s is comfortably above the ~100 s real-session backend latency. A hard floor of 120 s is enforced at runtime so even a misconfigured `policy.yaml` cannot cap a single cheap classification below backend latency. |
